...
function getPostsData(srcDir, config, ops) {
  return __async(this, null, function* () {
    ...
    const fileContentPromises = files.reduce((prev, f) => {
      prev[f] = {
        contentPromise: import_node_fs.default.promises.readFile(f, "utf-8"),
        datePromise: (0, import_theme_shared.getFileLastModifyTime)(f)
      };
      return prev;
    }, {});
    for (const file of files) {
      ...
      const url = config.site.base + (0, import_theme_shared.normalizePath)(import_node_path.default.relative(config.srcDir, file)).replace(/(^|\/)index\.md$/, "$1").replace(/\.md$/, config.cleanUrls ? "" : ".html");
      posts.push({
        filepath: file,
        fileContent,
